	...
	std::vector<std::string> comandospermitidos = {
	    ...
	    "10- base_remota <secuencia> <i> <j>",
	    "11- ayuda <comando> 				//*opcional",
	    "12- salir"
	};
	...
	void procesarComando(const std::string& linea,Genoma &genoma) {
		...
		std::stringstream ss(parametros);
		std::string nombreSec;
		int i, j, x, y;
		ss >> nombreSec >> i >> j >> x >> y;
		if (ss.fail()) {
			std::cout << "Error en los parametros. Uso: ruta_mas_corta <seq> i j x y\n";
		} else {
			genoma.ruta_mas_corta(nombreSec, i, j, x, y);
		}
		...
		ss >> nombreSec >> i >> j;
		if (ss.fail()) {
			std::cout << "Error en los parametros. Uso: base_remota <seq> i j\n";
		} else {
			genoma.base_remota(nombreSec, i, j);
		}
	...
	void ayudaComando(const std::string& comando) {
		...
		std::cout << "Busca la ruta de menor costo entre dos bases [i,j] y [x,y].\n";
	    } else if (comando == "base_remota") {
		std::cout << "Busca la misma base más lejana dada una posición inicial [i,j].\n";
	    }else {
